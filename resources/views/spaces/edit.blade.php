<x-app-layout>
  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="py-10">
      <form action="{{ route('spaces.update', $space->slug) }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="">
          <x-input-label for="title" :value="__('Название места')" />
          <x-text-input id="title" class="block mt-2 w-full" type="text" name="title" value="{{ $space->title }}" required />
          {{--
        ...
        <div class="mt-5">
          ...
          <textarea id="description"
            class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="textarea" rows="3" name="description" required>{{ old('description', trim($space->description)) }}</textarea>
          {{--
          <x-input-error :messages="$errors->get('email')" class="mt-2" /> --}}
        </div>
        <div class="mt-5">
        ...
        <div class="mt-5">
          ...
          <x-text-input id="address" class="block mt-2 w-full" type="text" name="address" value="{{ $space->address }}" required />
          {{--
          <x-input-error :messages="$errors->get('email')" class="mt-2" /> --}}
        </div>
        <div class="mt-5">
          <x-input-label for="phone" :value="__('Телефон')" />
          <x-text-input id="phone" class="block mt-2 w-full" type="tel" name="phone" value="{{ $space->phone }}" required />
          {{--
          <x-input-error :messages="$errors->get('email')" class="mt-2" /> --}}
        </div>
        <div class="mt-5">
          <x-input-label for="email" :value="__('Почта')" />
          <x-text-input id="email" class="block mt-2 w-full" type="email" name="email" value="{{ $space->email }}" required />
          {{--
          <x-input-error :messages="$errors->get('email')" class="mt-2" /> --}}
        </div>
        <div class="mt-5">
          <x-input-label for="website" :value="__('Сайт')" />
          <x-text-input id="website" class="block mt-2 w-full" type="url" name="website" value="{{ $space->website }}" required />
          {{--
          <x-input-error :messages="$errors->get('email')" class="mt-2" /> --}}
        </div>
        <fieldset class="flex gap-4 mt-5 border border-gray-300 p-4 rounded-md">
          ...
          <div class="w-1/2">
            ...
            <x-text-input id="latitude" class="block mt-2 w-full" type="text" name="latitude" value="{{ $space->latitude }}" required />
          </div>
          <div class="w-1/2">
            <x-input-label for="longitude" :value="__('Долгота')" />
            <x-text-input id="longitude" class="block mt-2 w-full" type="text" name="longitude" value="{{ $space->longitude }}" required />
          </div>
        </fieldset>
